...
class MyTestCase(unittest.TestCase):
    ...
    def test_reset(self, mock_post):
        ...
        self.assertEqual(Credentials(b).reset(), data, "should return an object")

    @patch('requests.post')
    def test_verify_single(self, mock_post):
        data = responses["single_bvn"]
        mock_post.return_value = R(body["single_bvn"])
        self.assertEqual(Bvn(b).verify_single({"body": {"BVN": "12345678901"}, "Aes_key": aes, "Iv_key": iv}), data,
    ...
    def test_multiple_bvn(self, mock_post):
        ...
        self.assertEqual(Bvn(b).verify_multiple(
            {"bvns": {"BVNS": "1234567890 1, 12345678902, 12345678903"}, "Aes_key": aes, "Iv_key": iv}), data,
                         "should return object")
    ...
    def test_watchlisted(self, mock_post):
        ...
        self.assertEqual(Bvn(b).bvn_watchlisted({"body": {"BVN": "12345678901"}, "Aes_key": aes, "Iv_key": iv}), output,
    ...
    def test_placeholder_reset(self, mock_post):
        data = {'aes_key': aes, 'password': "fdgfudkjd", 'ivkey': iv}
        mock_post.return_value = R("")
        self.assertEqual(Record(b).bvn_placeholder_reset(), data, "should return an object")

    @patch('requests.post')
    def test_validate_record(self, mock_post):
        response = responses["record"]
        mock_post.return_value = R(body["record"])
        self.assertEqual(Record(b).validate_record({
            "body": {
                "BVN": "12345678901",
                "FirstName": "Uchenna",
                "LastName": "Okoro",
    ...
    def test_validate_records(self, mock_post):
        ...
        self.assertEqual(Record(b).validate_records({"body": [
            {
                "BVN": "12345678901",
                "FirstName": "Uchenna",
                "LastName": "Okoro",
    ...
    def test_verify_finger_print(self, mock_post):
        ...
        self.assertEqual(FingerPrint(b).verify_fingerprint({"body": {
            "BVN": "12345678901",
